...
type Horoscope = { reading: string; focusWord: string; date: string }
...
export default function HoroscopeCard() {
  ...
  useEffect(() => {
    fetch("/api/horoscope")
      .then((r) => r.json())
      .then((data) => { if (data?.reading) setHoroscope(data); setLoading(false) })
    ...
    <div className="bg-white border border-[#e2dbd3] p-7 flex flex-col gap-5">
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="font-serif text-xl font-light text-stone-900">Libra Sun · Virgo Rising</p>
        </div>
